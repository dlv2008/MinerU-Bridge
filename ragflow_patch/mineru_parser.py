    ...
    def _read_output(self, output_dir: Path, file_stem: str, method: str = "auto", backend: str = "pipeline") -> list[
        ...
        for candidate_dir in candidate_dirs:
            for candidate_name in (f"{file_stem}_content_list.json", f"{safe_stem}_content_list.json"):
                ...
                self.logger.info(f"[MinerU] Trying candidate path: {candidate}")
        ...
        # Official MinerU 3.x layouts may place content_list.json under method-specific
        # folders or other nested directories. Fall back to a bounded recursive search.
        if not json_file:
            ...
            if recursive_candidates:
                ...
                self.logger.info(f"[MinerU] Found output via recursive search: {json_file}")
